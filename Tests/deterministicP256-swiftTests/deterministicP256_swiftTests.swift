...
class DeterministicP256Test: XCTestCase {
    ...
    func testInvalidSeedPhrases() {
        XCTAssertThrowsError(try D.genDerivedMainKeyWithBIP39(
            ...
            XCTAssertEqual(
                error as? MnemonicSwift.MnemonicError,
                MnemonicSwift.MnemonicError.checksumError
            )
        }
        ...
        XCTAssertThrowsError(try D.genDerivedMainKeyWithBIP39(
            ...
            XCTAssertEqual(
                error as? MnemonicSwift.MnemonicError,
                MnemonicSwift.MnemonicError.unsupportedLanguage
            )
        }
    }
    ...
    func testGenDerivedMainKeyThrowsError() {
        ...
        XCTAssertThrowsError(try D.genDerivedMainKey(
            entropy: [],
            salt: [],
            iterationCount: 0,
            keyLengthBytes: 32
        ...
        XCTAssertThrowsError(try D.genDerivedMainKeyWithBIP39(
            phrase: "salon zoo engage submit smile frost later decide wing sight chaos renew lizard rely canal coral scene hobby scare step bus leaf tobacco slice",
            salt: Array("liquid".utf8), iterationCount: 210_000, keyLength: 511
        )) { error in
            // Verify that the error is of the expected type and domain
            let nsError = error as NSError
    ...
    func testKeyPairGeneration() throws {
        ...
        // Test getPurePKBytes is the exact same as the hardcoded value, which
        // needs to match what
        // is defined in the Kotlin implementation
        XCTAssertEqual(
        ...
        // Test to ensure that the SHA256 hash of the public key is the same as
        // the hardcoded value
        // This is important because we calculate the FIDO2 Credential ID wiht
        // the SHA256 hash of
        // the public key,
        // and we need to be sure that it is detereministically produced the
        // same way across
        // implementations.
        // The hardcoded value must be the same as in the Kotlin implementation.
        let credentialId = SHA256.hash(data: D.getPurePKBytes(keyPair: keyPair))
        ...
        // Note that ECDSA signatures are non-deterministic (see ECDSA
        // nonce-reuse attack) so they
        // cannot be compared across rounds of tests.
        ...
        let isInvalidSignature = keyPair.publicKey.isValidSignature(
            signature,
            for: message + Data([0])
        )
        ...
        // The following is a raw representation of a signature produced by the
        // Kotlin library
        let kotlinEncodedSignatureRaw = Data([
            65,
            164,
            226,
            18,
        ...
        let kotlinSignatureRaw = try P256.Signing
            .ECDSASignature(rawRepresentation: kotlinEncodedSignatureRaw)
        let isValidKotlinSignatureRaw = keyPair.publicKey.isValidSignature(
            kotlinSignatureRaw,
            for: message
        )
        XCTAssertTrue(
            isValidKotlinSignatureRaw,
            "Kotlin Signature should be valid!"
        )

        // The following is a Distinguished Encoding Rules (DER) presentation of
        // a signature
        // produced by the Kotlin library
        let kotlinEncodedSignatureDER = Data([
            48,
            70,
            2,
            33,
        ...
        let kotlinSignatureDER = try P256.Signing
            .ECDSASignature(derRepresentation: kotlinEncodedSignatureDER)
        let isValidKotlinSignatureDER = keyPair.publicKey.isValidSignature(
            kotlinSignatureDER,
            for: message
        )
        XCTAssertTrue(
            isValidKotlinSignatureDER,
            "Kotlin Signature should be valid!"
        )
    }
}

// Makes MnemonicError from MnemonicSwift equatable for testing, so it can be
// used in XCTAssertEqual
extension MnemonicSwift.MnemonicError: Equatable {
    public static func == (lhs: MnemonicSwift.MnemonicError,
                           rhs: MnemonicSwift.MnemonicError) -> Bool
    {
        switch (lhs, rhs) {
